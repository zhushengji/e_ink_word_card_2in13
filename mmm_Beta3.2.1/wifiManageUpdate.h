...
void drawServiceWindow() {
  ...
  display.drawRect(windowX - 10, windowY - 15, 145, 90, GxEPD_BLACK);
  u8g2Fonts.setCursor(windowX-5, windowY);
  u8g2Fonts.print("WIFI配置服务");
  u8g2Fonts.setCursor(windowX-5, (windowY = windowY + 25));
  u8g2Fonts.print("IP地址: ");
...
//      Serial.println("Writing ssid to EEPROM...");
      WiFi.persistent(true);//需要保存wifi密码
    ...
    webServer.onNotFound([]() {
      String s = "<h1>配网模式</h1><p><a href=\"/settings\">点击配网</a></p><p><a href=\"/updatePage\">OTA 系统升级</a></p><p><a href=\"/upload\">文件上传</a></p>";
...
void handleFileUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    String f_story = webServer.arg("story");
    //    Serial.print("标记位测试点: "); Serial.println(f_story);
    if (!filename.startsWith("/")) filename = "/" + filename;
    if (!f_story.equals("是小说")) {
      is_story = false;
      filename = "/study" + filename;
    }
    Serial.print("上传文件名: "); Serial.println(filename);
