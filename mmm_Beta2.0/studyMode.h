...
void draw_menue(String filename) {
  int rect_count = 1;
  int mode_count = 1;
  //菜单
  ...
  display.drawRect(1, 25, 77, 62, GxEPD_BLACK);
...
File dataFile = SPIFFS.open(selsectbook(), "r");//("/words.txt", "r");
File wordsFile ;
//同步上次阅读进度
bool synchro() {
  bool s_flag = false;
...
void change_word(String filename, int read_flag, String line, String tem_line) {
  Serial.println("切换单词");
  //如果是短按要判断是阅读模式还是默写模式
  read_flag++;
  //同步进度
...
void add_word(String filename, String tem_line) {
  delay(400);
  display.setPartialWindow(0, 100, 255, 22);//局刷提示字样
  display.fillScreen(GxEPD_WHITE);
  u8g2Fonts.setFont(chinese_city_gb2312);
      ...
      while (true) {
        ...
        if ( digitalRead(key3) == LOW) {
          ...
          if (digitalRead(key3) != LOW) {
            /*短按*/
            //切换单词
            change_word(filename, read_flag, line, tem_line);
            break;
          }
          delay(300);
          //如果是长按，唤出菜单
